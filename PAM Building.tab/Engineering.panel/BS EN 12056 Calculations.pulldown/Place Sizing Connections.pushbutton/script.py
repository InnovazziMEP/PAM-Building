...
keywords = ['Foul Drainage Sizing Connection']
...
if not len(matching_families) > 0:
    forms.alert(
        ...
        title='Load SGPAMUK_Foul Drainage Sizing Connection'
    )
    script.exit()
                ...
                if run_button_clicked:  # Only proceed if the Run button was clicked
                    if selected_data:
                        ...
                        with TransactionGroup(doc, __title__) as tg:
                            ...
                            with Transaction(doc, "Add SGPAMUK_Foul Drainage Sizing Connection to Routing Preferences") as t1:
                                t1.Start()
                                ...
                                with Transaction(doc, "Remove SGPAMUK_Foul Drainage Sizing Connection from Routing Preferences") as t3:
                                    t3.Start()
                                    try:
